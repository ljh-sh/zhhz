#!/usr/bin/env bash
# Cache-line + arena bench runner — v0.8.0-cache-line branch.
# Measures zhhz fast + trigram vs opencc 1.3.1 on 8 real corpora + 1 repeat.
set -euo pipefail

ZHHZ_BIN="${ZHHZ_BIN:-/tmp/zhhz-cache-target/release/examples/perf_only}"

CORPORA=(
  "realistic-repeat /tmp/corpus-realistic-repeat.txt"
  "realistic-norepeat /tmp/corpus-realistic-norepeat.txt"
  "realistic-var /tmp/corpus-realistic-var.txt"
  "jinghuayuan /tmp/corpus-jinghuayuan.txt"
  "shuotang /tmp/corpus-shuotang.txt"
  "mudanting /tmp/corpus-mudanting.txt"
  "liezi /tmp/corpus-liezi.txt"
  "yangjiajiang /tmp/corpus-yangjiajiang.txt"
...
ipc = instr / cyc
print(f"{label:<32} {mode:<8} {mbps:10.2f} MB/s  instr/conv={instr_per:.2e}  cyc/conv={cyc_per:.2e}  IPC={ipc:.2f}")
PY
    rm -f "$rawfile"
}
...
print(f"{label+' (opencc 1.3.1)':<32} {'fast':<8} {avg_mbps:10.2f} MB/s  instr/conv={avg_instr:.2e}  cyc/conv={avg_cyc:.2e}  IPC={ipc:.2f}")
PY
    rm -f "$tmpf" "$rawfile"
}

echo "=== cache-line + arena bench ==="
...
echo "binary: $ZHHZ_BIN"
echo ""
printf "%-32s %-8s %-22s %-22s %s\n" "binary" "mode" "throughput" "instr/convert" "IPC"
echo "--------------------------------------------------------------------------------------------------------"
...
for entry in "${CORPORA[@]}"; do
    ...
    path=${entry##* }
    echo "--- corpus: $label ---"
    run_perf_only "$ZHHZ_BIN" fast    "$path" "zhhz fast"
    run_perf_only "$ZHHZ_BIN" trigram "$path" "zhhz trigram"
